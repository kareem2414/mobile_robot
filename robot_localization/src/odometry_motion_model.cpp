#include "robot_localization/odometry_motion_model.hpp"
    ...
    is_first_odom_(true)
{
    ...
    if (nr_samples_ > 0)
    {
        ...
        RCLCPP_FATAL_STREAM(get_logger(), "Invalid number of samples requisted " << nr_samples_);
        return;
    }

    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("robot_controller/odom", 10, std::bind(&OdometryMotionModel::odomCallback, this, _1));
    pose_array_pub_ = create_publisher<geometry_msgs::msg::PoseArray>("robot_controller/samples",10);
}
...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry & odom)
{
    ...
    double odom_x_increament = odom.pose.pose.position.x - last_odom_x_;
    double odom_y_increament = odom.pose.pose.position.y - last_odom_y_;
    double odom_theta_increament = angle_diff(yaw, last_odom_theta_);

    double delta_rot1 = 0.0;
    double delta_trans = sqrt(std::pow(odom_y_increament,2)+std::pow(odom_x_increament,2));
    if (delta_trans > 0.01){
        delta_rot1 = angle_diff((odom_y_increament,odom_x_increament), yaw);
    }
    double delta_rot2 = angle_diff(odom_theta_increament, delta_rot1);
